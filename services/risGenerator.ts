...
const generateRisPdf = (
    ...
    for (let page = 0; page < totalPages; page++) {
        ...
        const colWidths = {
            stockNo: 12, unit: 15, description: 73, reqQty: 16,
            yes: 8, no: 8, issueQty: 16, remarks: 46
        };
        ...
        autoTable(doc, {
            ...
            theme: 'grid',
            styles: {
                fontSize: 8, cellPadding: 0.75, minCellHeight: 5.5,
                lineColor: [0, 0, 0], lineWidth: 0.15, textColor: [0, 0, 0], valign: 'middle'
            },
            headStyles: {
                fillColor: [255, 255, 255], textColor: [0, 0, 0], fontStyle: 'bold', lineWidth: 0.15,
                minCellHeight: 5.5, halign: 'center', valign: 'middle'
            },
            columnStyles: {
        ...
        if (signatures) {
            const renderSig = (url: string | undefined, x: number, y: number, w: number, h: number) => {
                if (url) {
                    try {
                        const sigWidth = 35; // Slight reduction to ensure fit
                        const sigHeight = 10;
            ...
            renderSig(signatures.approverSigUrl, col2X, finalY, colWidth, sigRowHeight);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7);
        centerText(signatures?.requesterDesignation || '', col1X, colWidth, finalY + 3.5, false);
        ...
        centerText(signatures?.receiver ? (signatures.receiverDesignation || '') : (signatures?.requesterDesignation || ''), col4X, colWidth, finalY + 3.5, false);
        ...
        centerText(formatDate(dates?.verified || dates?.requested), col2X, colWidth, finalY + 3.5);
        centerText(formatDate(dates?.issued || dates?.requested), col3X, colWidth, finalY + 3.5);
...
export const generateRIS = async (request: any, users: User[]) => {
    // 1. Map Items
    ...
    const dates = {
        ...
        issued: ['For Issuance', 'To Receive', 'History'].includes(request.status) ? today : undefined,
        received: ['To Receive', 'History'].includes(request.status) ? today : undefined,
    };

    // 4. Call Generator
    generateRisPdf(items, request.purpose, request.id, signatures, dates, 'print');
};
